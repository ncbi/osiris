...
void StructuredErrorFunction(void *userData, xmlErrorPtr error)
{
  fprintf(stderr,"ERROR %d\nLINE %d\n  %s\n",error->code,error->line,error->message);
}

void GenericErrorFunction(void *ctx, const char *msg,...)
{
  char ch[8192];
  int n;
  va_list ap;
  va_start(ap,msg);
  n = vsnprintf(ch,sizeof(ch) - 1,msg,ap);
  va_end(ap);
  fprintf(stderr,"%s\n",ch);
}
void GenericErrorFunction2(void *ctx, const char *msg,...)
{
  char ch[8192];
  int n;
  va_list ap;
...
main(int argc, char **argv)
{
  ...
  xsltSetGenericErrorFunc(NULL,GenericErrorFunction2);
  /* end djh */
