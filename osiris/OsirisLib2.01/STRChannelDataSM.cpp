...
int STRSampleChannelData :: AnalyzeDynamicBaselineUsingDerivativeFilterAndNormalizeRawDataSM (int startTime, double reportMinTime) {

	//
	//	Sample Stage 1
	//	Similar to other normalization, but use derivative filter instead of preliminary curve fits.
	//
...
//	cout << "Start time = " << startTime << endl;
	int numSamples = mData->GetNumberOfSamples ();
	CalculateFirstDerivativeFilter ();

	ProspectiveIntervalForNormalization* nextProspectiveInterval;
	NormalizationInterval* nextNormalizationInterval;
	list<ProspectiveIntervalForNormalization*> prospectiveList;
	list<NormalizationInterval*> normalizationIntervalList;

	int nextStartTime = startBaselineFit;
	int nextEndTime;

	while (true) {

		nextProspectiveInterval = new ProspectiveIntervalForNormalization (nextStartTime, mData, mDerivFilter, filterHeight);
		nextEndTime = nextProspectiveInterval->GetEnd ();

		if (nextProspectiveInterval->IsLongEnough (minIntervalLength))
			prospectiveList.push_back (nextProspectiveInterval);

		else
			delete nextProspectiveInterval;

		nextStartTime = nextEndTime + 1;

		if (numSamples - nextStartTime < minIntervalLength)
			break;
	}

	if (prospectiveList.size () == 0) {

		cout << "No intervals of sufficient length (" << minIntervalLength << ") in channel " << mChannel << "\n";
		return -1;
	}

	// ****Add division into subintervals and creating knots here...
