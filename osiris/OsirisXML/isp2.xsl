...
#  Description:  Export file for the Illinois State Police
#    for OSIRIS data export
#
-->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="searchString">
    <xsl:text>:CSF1PO:6,15</xsl:text>
    <xsl:text>:D13S317:8,15</xsl:text>
    <xsl:text>:D16S539:5,15</xsl:text>
    <xsl:text>:D18S51:9,26</xsl:text>
    <xsl:text>:D21S11:24.2,38</xsl:text>
    <xsl:text>:D3S1358:12,19</xsl:text>
    <xsl:text>:D5S818:7,16</xsl:text>
    <xsl:text>:D7S820:6,14</xsl:text>
    <xsl:text>:D8S1179:8,19</xsl:text>
    <xsl:text>:FGA:18,30</xsl:text>
    <xsl:text>:Penta D:2.2,17</xsl:text>
    <xsl:text>:Penta E:5,24</xsl:text>
    <xsl:text>:TH01:5,10</xsl:text>
    <xsl:text>:TPOX:6,13</xsl:text>
    <xsl:text>:vWA:11,21</xsl:text>
    <!-- end with a colon -->
    <xsl:text>:</xsl:text>
  </xsl:variable>
  ...
  <xsl:template name="getLocusRange">
    <xsl:param name="locus"/>
    <xsl:variable name="sLocus" select="concat(':',$locus,':')"/>
    <xsl:variable name="range">
      <xsl:variable name="r1">
        <xsl:choose>
          <xsl:when test="contains($searchString,$sLocus)">
            <xsl:value-of select="substring-after($searchString,$sLocus)"/>
          </xsl:when>
          <xsl:otherwise>
            <!--
            <xsl:message>
              <xsl:text>cannot find range for locus: </xsl:text>
              <xsl:value-of select="$locus"/>
            </xsl:message>
            -->
            <xsl:text>0,9999:</xsl:text>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:variable>
  ...
  <xsl:template name="chooseUD">
    ...
    <xsl:param name="concat" select="0"/>
    <xsl:param name="maxlen" select="255"/>
    <xsl:variable name="n">
    ...
    <xsl:choose>
      ...
      <xsl:when test="($nNew = $n) and ($concat != '') and ($concat != 'false') and (string(number($concat)) != '0')">
        <xsl:variable name="s1" select="substring-after($UD,':')"/>
        <xsl:variable name="s2" select="substring-after($UDnew,':')"/>
        
        <xsl:variable name="sConcat">
          <xsl:value-of select="$s1"/>
          <xsl:if test="$s1 != '' and $s2 != ''">
            <xsl:text>;</xsl:text>
          </xsl:if>
          <xsl:value-of select="substring-after($UDnew,':')"/>
        </xsl:variable>
        <xsl:value-of select="$n"/>
        <xsl:value-of select="':'"/>
        <xsl:value-of select="substring($sConcat,1,$maxlen)"/>
      </xsl:when>
      <xsl:otherwise>
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:call-template name="chooseUD">
          ...
          <!-- concatentate messages for UD3 -->
          <xsl:with-param name="concat" select="1"/>
        </xsl:call-template>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:variable name="check123">
          ...
          <xsl:variable name="channelNr">
            <xsl:choose>
              <xsl:when test="$checkChannel = 2">
                <xsl:value-of select="$n/../../ChannelNr"/>
              </xsl:when>
              <xsl:when test="$checkChannel = 1">
                <xsl:value-of select="$n/../ChannelNr"/>
              </xsl:when>
              <xsl:when test="name($n/..) = 'ILSAlerts'">
                <xsl:value-of
                  select="/OsirisAnalysisReport/Heading/ILSchannelNr"/>
              </xsl:when>
              <xsl:otherwise>
          ...
          <xsl:choose>
            <xsl:when test="$channelNr &gt; 0">
              <xsl:call-template name="str-replace">
                <xsl:with-param name="str" select="$tmp1"/>
        ...
        <xsl:call-template name="getUD123">
          ...
          <xsl:with-param name="checkChannel" select="$checkChannel"/>
        </xsl:call-template>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="formatUDs">
    <xsl:param name="str"/>
    <xsl:param name="sampleName" select="$varSample"/>
    <xsl:param name="locusName" select="$varLocus"/>
    <xsl:param name="alleles" select="$varAlleles"/>
    ...
    <xsl:choose>
      <xsl:when test="contains($str,'$(')">
        ...
        <xsl:choose>
          <xsl:when test="string($alleles) = $varAlleles">
            <xsl:value-of select="$formatLocus"/>
          </xsl:when>
          <xsl:when test="contains($formatLocus,$varAlleles)">
  ...
  <xsl:template name="locusUD">
    ...
    <xsl:call-template name="formatUDs">
      <xsl:with-param name="str" select="$set3"/>
      <xsl:with-param name="locusName" select="$locus/LocusName"/>
      <xsl:with-param name="alleles" select="$alleles"/>
    </xsl:call-template>
  </xsl:template>

  <xsl:template name="lociUD">
    <xsl:param name="sample"/>
    <xsl:param name="ndx" select="1"/>
    <xsl:param name="count" select="count($loci)"/>
    <xsl:param name="ud" select="$DEFAULT123"/>

    <xsl:choose>
      <xsl:when test="$ndx &gt; $count">
        <xsl:value-of select="$ud"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:variable name="udNext">
          <xsl:variable name="locusName" select="$loci[$ndx]"/>
          <xsl:variable name="locus"
            select="$sample/Locus[LocusName = $locusName]"/>

          <xsl:choose>
            <xsl:when test="$locus">
              <xsl:variable name="udThisLocus">
                <xsl:call-template name="locusUD">
                  <xsl:with-param name="locus" select="$locus"/>
                </xsl:call-template>
              </xsl:variable>
              <xsl:call-template name="chooseUDs">
                <xsl:with-param name="UDs" select="$ud"/>
                <xsl:with-param name="UDsnew" select="$udThisLocus"/>
              </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
              <xsl:value-of select="$ud"/>
        ...
        <xsl:call-template name="lociUD">
          <xsl:with-param name="ud" select="$udNext"/>
  ...
  <xsl:template name="sampleUD">
    ...
    <xsl:variable name="out">
      <xsl:call-template name="chooseUDsPlus">
        ...
        <xsl:with-param name="UDs5" select="$set5"/>
        <xsl:with-param name="count" select="5"/>
      </xsl:call-template>
    </xsl:variable>
    <xsl:call-template name="formatUDs">
      <xsl:with-param name="str" select="$out"/>
    ...
    <xsl:choose>
      <xsl:when test="not($locus = 'AMEL')">
        <xsl:variable name="minmax">
          <xsl:call-template name="getLocusRange">
            <xsl:with-param name="locus" select="$locus"/>
          </xsl:call-template>
        </xsl:variable>
        <xsl:variable name="min" 
          select="number(substring-before($minmax,','))"/>
        <xsl:variable name="max"
          select="number(substring-after($minmax,','))"/>
        ...
        <xsl:if test="$enabled != 0">
          ...
          <xsl:if test="not($UD123sample = $DONOTEXPORT3)">
            ...
            <xsl:variable name="UDformatted">
              <xsl:call-template name="extractUDs">
                <xsl:with-param name="UDs" select="$UD123sample"/>
              </xsl:call-template>
            </xsl:variable>
