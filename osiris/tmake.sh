...
if test ! -r "$ERRFILE"; then
  for x in OpenFolder/bin fsa2xml/bin TestAnalysisDirectoryLCv2.11/bin OsirisAnalysis/bin GenerateLadderFile/bin BuildStandardControlFile/bin GenerateILSFamily/bin; do
    MAKE $x > make${f}.txt 2>&1 &
    f=`expr $f + 1`
  done
  wait
